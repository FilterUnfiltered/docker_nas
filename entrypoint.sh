...
if [ -f "/id_rsa" ]; then
  ...
  chmod 600 /id_rsa  # ensure correct permissions
else
...
sshfs -o IdentityFile=/id_rsa -o StrictHostKeyChecking=no \
  "$SSHFS_REMOTE:$SSHFS_REMOTE_PATH" "$SSHFS_LOCAL_PATH"

echo "Mounting gocryptfs -> $GOCRYPTFS_MOUNT"
echo "$GOCRYPTFS_PASSWORD" > gocryptfs_pass
...
if [ ! -f "$SSHFS_LOCAL_PATH/gocryptfs.conf" ]; then
  ...
  gocryptfs -init -passfile gocryptfs_pass "$SSHFS_LOCAL_PATH"
else
  echo "Found existing gocryptfs.conf, using it."
fi

gocryptfs -passfile gocryptfs_pass -nosyslog "$SSHFS_LOCAL_PATH" "$GOCRYPTFS_MOUNT"

shred -u gocryptfs_pass
